...
def Conv2d_Stacks(channels,bn=True):
    '''
    function
    ...
    for out_channel in channels[1:-1]:
        layers.append(Conv2d(last_channel, out_channel, if_bn=bn))
        last_channel = out_channel
    layers.append(Conv2d(last_channel,channels[-1], if_bn=False, activation_fn=None))
...
def MLP_Stacks(channels,bn=True):
    '''
    function    
        stack multiple layers of mlp based on input channel list
    input  
...
def sample_and_group(xyz, points, npoint, nsample, radius, use_xyz=True, use_residual=True):
    """
    Args:
        xyz: Tensor, (B, 3, N)
        points: Tensor, (B, f, N)
...
class PointNet_FP_Module(nn.Module):
    def __init__(self, in_channel, new_point_channel, mlp, if_bn=True):
        """
        Args:
    ...
    def forward(self, new_xyz, xyz, new_points, points):
        """
        Args:
            new_xyz: Tensor, (B, 3, N)
            xyz: Tensor, (B, 3, M)
            new_points: Tensor, (B, in_channel, N)
            points: Tensor, (B, in_channel, M)

        Returns:MLP_CONV
            new_points: Tensor, (B, mlp[-1], N)
        """
        dist, idx = three_nn(new_xyz.permute(0, 2, 1).contiguous(), xyz.permute(0, 2, 1).contiguous())
        dist = torch.clamp_min(dist, 1e-10)  # (B, N, 3)
        recip_dist = 1.0/dist
        norm = torch.sum(recip_dist, 2, keepdim=True).repeat((1, 1, 3))
        weight = recip_dist / norm
        interpolated_points = three_interpolate(points, idx, weight) # B, in_channel, N

        out_points = torch.cat([interpolated_points, new_points], 1)
        out_points = self.mlp_conv(out_points)
        return out_points
...
def query_knn_in_range(nsample, xyz, new_xyz, radius, include_self=True):
    """Find k-NN of new_xyz in xyz"""
    #B,N,3 | B,S,3
    pad = 0 if include_self else 1
    sqrdists = square_distance(new_xyz, xyz)  # B, S, N
    sorted_dist, indices = torch.sort(sqrdists, dim=-1, descending=False) #B,N,K


    B,N,_ = new_xyz.shape
    idx = indices[:, :, pad: nsample+pad]
    mask = torch.sqrt(sorted_dist[:, :, pad: nsample+pad]) > radius
    indices_self = torch.arange(xyz.shape[1], xyz.shape[1]+new_xyz.shape[1]).unsqueeze(0).unsqueeze(-1).repeat(B,1,nsample).to(mask.device)
    idx[mask] = indices_self[mask]
    #sdist = sorted_dist[:,:,pad: nsample+pad]
    return idx.int()

def sample_and_group_knn_for_density(new_xyz, xyz, nsample, feats=None,  include_self=False):
    """Find k-NN of new_xyz in xyz"""
    #B,3,N
    pad = 0 if include_self else 1
    xyz_flipped = xyz.permute(0, 2, 1).contiguous()  # (B, N, 3)
    new_xyz_flipped = new_xyz.permute(0, 2, 1).contiguous()  # (B, N, 3)
    sqrdists = square_distance(new_xyz_flipped, xyz_flipped)  # B, S, N
    sorted_dist, indices = torch.sort(sqrdists, dim=-1, descending=False)
    idx = indices[:, :, pad: nsample+pad].int()
    grouped_xyz = grouping_operation(xyz, idx) # (B, 3, npoint, nsample)
    grouped_xyz -= new_xyz.unsqueeze(3).repeat(1, 1, 1, nsample)

    if feats is not None:
        grouped_feats = grouping_operation(feats, idx) # (B, f, npoint, nsample)
    else:
        grouped_feats = grouped_xyz

    return grouped_xyz, grouped_feats, idx,  sorted_dist[:,:,:nsample]

def sample_and_group_knn(xyz, points, npoint, k, use_xyz=True, idx=None, new_xyz = None, include_self = True):
    """
    Args:
        xyz: Tensor, (B, 3, N)
        points: Tensor, (B, f, N)
    ...
    if new_xyz is None:
        ...
        if new_xyz.shape[-1]==3:
            new_xyz = new_xyz.permute(0, 2, 1).contiguous() # (B, 3, N)

    if idx is None:
        idx = query_knn(k, xyz_flipped, new_xyz.permute(0, 2, 1).contiguous(), include_self=include_self)
    grouped_xyz = grouping_operation(xyz, idx) # (B, 3, npoint, nsample)
...
class UpLayer_mine(nn.Module):
    ...
    def __init__(self, dim, seed_dim, up_factor=2, i=0, radius=1, n_knn=20, interpolate='three', attn_channel=True):
        super(UpLayer_mine, self).__init__()
        self.i = i
        self.up_factor = up_factor
        self.radius = radius
        self.n_knn = n_knn
        ...
        if self.up_factor > 1:
            ...
            self.mlp_delta_feature = MLP_Res(in_dim=dim, hidden_dim=dim, out_dim=dim)
    ...
    def forward(self, pcd_prev, seed, seed_feat, K_prev=None):
        """
        Args:
            pcd_prev: last generated pcds
            seed: original seeds
            seed_feat: original feat
            K_prev: last generated feat
        Returns:
        ...
        # merge upsampled and original feat of pcd_prev -> Q
        ...
        if self.up_factor > 1:
            ...
            K_curr = self.mlp_delta_feature(H)
            pcd_new = pcd_prev
...
class UpLayer(nn.Module):
    ...
    Upsample Layer with upsample transformers
    """
    def __init__(self, dim, seed_dim, up_factor=2, i=0, radius=1, n_knn=20, interpolate='three', attn_channel=True, finetune=False):
    ...
    def forward(self, pcd_prev, seed, seed_feat, K_prev=None):
        ...
        # H = self.uptrans1(pcd_prev, K_prev if K_prev is not None else Q, Q, upfeat=feat_upsample) # (B, 128, N_prev)
        # feat_child = self.uptrans2(pcd_prev, K_prev if K_prev is not None else H, H, upfeat=feat_upsample) # (B, 128, N_prev*up_factor)
        feat_child = self.uptrans2(pcd_prev, K_prev if K_prev is not None else Q, Q, upfeat=feat_upsample) # (B, 128, N_prev*up_factor)
        if self.finetune:
            feat_child = self.finetune_layers(feat_child)
        # Get current features K
        # H_up = self.upsample(H)
        H_up = self.upsample(Q)
        K_curr = self.mlp_delta_feature(torch.cat([feat_child, H_up], 1))
...
class UpLayer_small(nn.Module):
    """
    Upsample Layer with upsample transformers
    """
    def __init__(self, dim, seed_dim, up_factor=2, i=0, radius=1, n_knn=20, interpolate='three', attn_channel=True):
        super(UpLayer_small, self).__init__()
        self.i = i
        self.up_factor = up_factor
        self.radius = radius
        self.n_knn = n_knn
