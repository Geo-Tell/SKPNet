...
def to_device(data):
    for key in data.keys():
        if not isinstance(data[key][0],str):
...
def seprate_point_cloud(xyz, crop, num_points=2048, fixed_points=None, padding_zeros=False, use_fps = False):
    '''
    ...
    if crop == n:
        output = []
        for points in xyz:
            output.append(fps(points.unsqueeze(0).clone(), num_points))
        output = torch.cat(output, dim=0)  # B N 3
    ...
    for points in xyz:
        ...
        #fps太慢，采取替代方案
        if use_fps:
            INPUT.append(fps(input_data, num_points))
        else:
            indices = torch.randint(0, input_data.size(1), (num_points,), device=input_data.device)
            input_data_ = input_data[0, indices, :].unsqueeze(0)
            # input_data_ = torch.concatenate([input_data, input_data[:,:(num_points-input_data.shape[1]),:]], dim=1) #1,n,3
            INPUT.append(input_data_)
...
def get_ptcloud_img(ptcloud, s=None):
    ...
    if s is not None:
        ax.scatter(x, y,z, s=s, zdir='z', c=x, cmap='jet')
    else:
...
def seprate_point_cloud_with_varying_densities(xyz, crop, num_points=2048, fixed_points=None, padding_zeros=False):
    '''
     seprate point cloud: usage : using to generate the incomplete point cloud with a setted number.
    '''
    _,n,c = xyz.shape
    ...
    for points in xyz:
        ...
        if isinstance(crop, list):
            INPUT.append(fps(input_data, num_points))
            CROP.append(fps(crop_data, num_points))
        else:
            INPUT.append(fps(input_data, num_points))
            CROP.append(fps(crop_data, num_points))
...
def copy_all_code(save_path,include_dirs=['models', 'tools','utils','datasets','cfgs']):
    #/external/pointnet2_ops_lib/pointnet2_ops
    src_dir = "./"
    dst_dir = os.path.join(save_path, "code")
